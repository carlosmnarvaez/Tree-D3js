...
d3.chart.architectureTree = function() {
    ...
    function chart(){
        if (typeof(tree) === 'undefined') {
            ...
            svg = d3.select("#graph").append("svg")
                ...
                .attr("transform", "translate(" + diameter / 2.75 + "," + diameter / 2.75 + ")");

        }

        // var canvasTwo = d3.select("#hola")
        //     .append("svg")
        //     .attr("width", 500)
        //     .attr("height", 500);
        // var circleTwo = canvasTwo.append("circleTwo")
        //     .attr("cx", 100)
        //     .attr("cy", 150)
        //     .attr("r", 50)
        //     .attr("fill", "red");
    ...
    var updateData = function(container, nodes, links) {
        ...
        node.append("circle")
            .attr("r", function(d) { return 4.5 * (d.size || 1.5); })
    ...
    var isFoundByFilter = function(d) {
        var i;
        if (!filters.name && !filters.technos.length) {
            // nothing selected
            return true;
        }
        if (filters.name) {
